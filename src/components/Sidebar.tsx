...
const Sidebar: React.FC<SidebarPropType> = ({ searchBtn, setSearchBtn, folderToggle, setFolderToggle, addNote, setAddNote, currFolderName, setCurrentFolderName,setNoteSearchInput, searchedNotes, setSearchedNotes, showSearchDropdown, setShowSearchDropdown}) => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const matchedNotes = allNotes.filter((note) => {
          const titleMatch = note.title ?.toLowerCase().includes(trimmedValue.toLowerCase());
        ...
        {searchBtn ? (
          ...
          <div className="relative w-65" onClick={(e) => e.stopPropagation()}>
            <div className="flex flex-row gap-2 p-2.5 justify-start items-center bg-(--searchBtnBg) h-10 cursor-pointer ">
              ...
              <input onChange={(e) => setInputValue(e.target.value)} value={inputValue} id="searchNote"
                className="font-semibold text-(--mainText) text-base outline-none w-full" type="text" placeholder="Search note" />
            </div>

            {showSearchDropdown && (
              <div onClick={(e) => e.stopPropagation()} className="absolute top-12 left-0 w-full max-h-80 overflow-y-auto no-scrollbar rounded-md bg-(--sidebarBg) border border-white/10 shadow-xl z-50">
                {searchedNotes.length > 0 ? (
                  searchedNotes.map((note) => (
                    <div key={note.id} onClick={() => handleSearchClick(note)}
                      className="px-4 py-3 cursor-pointer hover:bg-(--folderHoverBg2) border-b border-white/5" >
                      <h3 className="text-sm font-semibold text-(--mainText)"> {note.title}</h3>
                    </div>
                  ))
                ) : (
                  <div className="px-4 py-3 text-sm text-(--middleText)">Note not found</div>
                )}
              </div>
            )}
          </div>
