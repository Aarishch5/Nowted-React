...
const Recents: React.FC = () => {
  const { recentNotes, setRecentNotes, selectedRecentNotesId, setSelectedRecentNotesId, currSelectedNotesId, setRecentFolderId } = useContext(UserContext);

  const recentDataAPI: string =
      ...
      <div className="flex flex-col gap-1.25 w-full">
        {recentNotes.map((elem) => (
          <div
            ...
            className={`hover:bg-[#312EB5] h-10 w-full flex flex-row gap-3.75 cursor-pointer items-center px-5 text-base ${elem.id === selectedRecentNotesId && elem.id === currSelectedNotesId ? "bg-[#312EB5] text-white" : "text-[#FFFFFF99]"}  hover:text-white`}
          >
